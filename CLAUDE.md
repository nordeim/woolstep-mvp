...
WOOLSTEP MVP is a single-page e-commerce application showcasing premium merino wool sneakers designed for Singapore's tropical climate. The project serves as a functional prototype demonstrating modern React 19 features (`useActionState`, `useOptimistic`), TypeScript 5.9's `erasableSyntaxOnly` flag, and Vite 8's Rolldown bundler.
...
# Generate route tree (run after adding/modifying routes)
npx tsr generate
```
...
- **Test file location**: `src/test/*.test.ts` or `src/test/*.test.tsx`
...
- **Use `describe/it/expect`** from `vitest` (not Jest globals)
- **Factory pattern for test data**: `getMockProduct(overrides)`

```typescript
// ✅ Correct - Store test pattern
...
describe('CartStore', () => {
  ...
  it('should add item to cart', () => {
    const { addItem, getCount } = useCartStore.getState()
    addItem({ id: 'test-1', name: 'Test', price: 100, image: 'test.jpg', category: 'men' })
...
```bash
git commit -m "feat: implement useOptimistic for favorites toggle"
...
│   ├── sections/     # Page sections (Hero, ProductGrid, Newsletter)
│   └── cart/         # CartPanel, CartItem, FavoriteButton
├── stores/           # Zustand stores (cartStore, productStore, toastStore)
